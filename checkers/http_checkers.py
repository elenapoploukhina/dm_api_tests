    ...
    try:
        ...
        if expected_status_code != requests.codes.OK:
            raise AssertionError(
                f"Ожидаемый статус код должен быть равен {expected_status_code}, но запрос прошел успешно."
            )
        if expected_message:
        ...
        assert e.response.status_code == expected_status_code, \
            f"Ожидаемый статус код - {expected_status_code}, получен статус код - {e.response.status_code}."
        assert e.response.json()["title"] == expected_message, \
            f"Ожидаемое сообщение - '{expected_message}', получено сообщение - '{e.response.json()['title']}'."
